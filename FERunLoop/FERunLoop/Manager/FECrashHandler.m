...
@interface FECrashHandler()

@end

static FECrashHandler *sharedCrashHandler;
static dispatch_once_t   onceToken;

@implementation FECrashHandler
...
- (void)handleException:(NSException *)exception {
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"程序崩溃了"
                                                   ...
                                                   delegate:self
                                          cancelButtonTitle:@"死就死吧"
                                          otherButtonTitles:@"回光返照", nil];
    [alert show];
...
void HandleException(NSException *exception) {
    ...
    FECrashHandler *crashObject = [FECrashHandler sharedInstance];
...
{
    // 这种情况的崩溃信息，就另某他法来捕获吧
    NSArray *callStack = [FECrashHandler backtrace];
